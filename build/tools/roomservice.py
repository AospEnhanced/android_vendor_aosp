...
if not depsonly:
    print("Device %s not found. Attempting to retrieve device repository from AospEnhanced Github (http://github.com/AospEnhanced)." % device)
...
if not depsonly:
    githubreq = urllib.request.Request("https://raw.githubusercontent.com/AospEnhanced/mirror/main/default.xml")
    try:
...
def get_default_revision():
    m = ElementTree.parse(".repo/manifests/snippets/aospenhanced.xml")
...
def add_to_manifest(repositories):
    ...
    for repository in repositories:
        ...
        if is_in_manifest(repo_target):
            print('AospEnhanced/%s already fetched to %s' % (repo_name, repo_target))
            continue

        project = ElementTree.Element("project", attrib = {
            "path": repo_target,
            "remote": "github",
            "name": "AospEnhanced/%s" % repo_name,
...
def get_default_or_fallback_revision(repo_name):
    ...
    githubreq = urllib.request.Request("https://api.github.com/repos/AospEnhanced/" + repo_name + "/branches")
    add_auth(githubreq)
...
print("Repository for %s not found in the AospEnhanced Github repository list. If this is in error, you may need to manually add it to your local_manifests/roomservice.xml." % device)
